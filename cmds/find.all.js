...
function func_fund_entry(fund){
	...
	req.on('error', function(err){
		util.log('Error func_fund_entry: ' + err);
	});
	req.end();
}
...
function func_list_process(list){
	for (var i = 0; i < list.length; i++){
		func_fund_entry(list[i]);
	}
}

function func_list_entry(res){
	var cmd='';
	res.on('data', function(chunk){
		cmd += chunk;
	});
	res.on('end', ()=>{
		cmd = cmd + 'r;';
		var fund_list = eval(cmd);
		func_list_process(fund_list);
	}); 
}

var req = http.request('http://fund.eastmoney.com/js/fundcode_search.js?v=20160914135859',  func_list_entry);
req.on('error', function(err){
	util.log('Error main: ' + err);
});
req.end();
